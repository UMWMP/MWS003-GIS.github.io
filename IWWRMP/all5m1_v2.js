...
function loadGeoJsonLayer(url, layerName) {
    fetch(url)
        ...
        .then(data => {
            // Create GeoJSON layer with custom style and click event
            const geoJsonLayer = L.geoJSON(data, {
                style: (feature) => {
                    // Determine the style based on layer name or land cover type
                    const landCoverType = feature.properties['class']; // Adjust this property as needed
                    return styleOptions[landCoverType] || styleOptions[layerName] || { color: "#333", weight: 1, fillOpacity: 0.5 }; // Default style
                },
                onEachFeature: (feature, layer) => {
                    // Add a click event to each feature in the GeoJSON layer
                    layer.on('click', () => updateInfoPanel(feature.properties));
                }
            });
